...
export const IrrigationModal = ({ isOpen, onClose, analysis, farmProfile, t }: any) => {
    ...
    // Translation Layer: mm -> Liters -> Hours
    const areaAcres = farmProfile?.total_area || 1;
    const areaM2 = areaAcres * 4046.86;
    const pumpLph = farmProfile?.pump_capacity_lph || 5000;
    const method = farmProfile?.irrigation_method || 'Drip';

    // Efficiency Factors: Drip (90%), Sprinkler (75%), Surface (60%)
    const efficiency = method === 'Drip' ? 0.9 : method === 'Sprinkler' ? 0.75 : 0.6;

    const totalLitersNeeded = (amountMm * areaM2) / efficiency;
